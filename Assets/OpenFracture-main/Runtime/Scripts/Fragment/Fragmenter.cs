...
public static class Fragmenter
{
                                ...
                                string saveFolderPath = "")
    {
		...
		// If set to use a defined axis and origin then use them for the first iteration
		if (options.useOrigin && options.useAxis)
		{
            FragmentData meshData = fragments.Dequeue();
            meshData.CalculateBounds();
			
            MeshSlicer.Slice(sourceObject,
							 meshData,
                             options.axis,
                             options.origin,
        ...
        while (fragments.Count < options.fragmentCount)
        {
			...
			// Get mesh bounds center in worldspace
			Vector3 boundsCenter = sourceObject.transform.TransformPoint(meshData.Bounds.center);
                                       ...
                                       ref int i)
    {
		...
		var parentVelocity = sourceObject.GetComponent<Rigidbody>().velocity;
		var parentAngularVelocity = sourceObject.GetComponent<Rigidbody>().angularVelocity;
        ...
        for(int k = 0; k < meshes.Length; k++)
        {
			...
			rigidBody.AddForce(parentVelocity * rigidBody.mass, ForceMode.Impulse);
			rigidBody.AddTorque(parentAngularVelocity * rigidBody.mass, ForceMode.Impulse);
